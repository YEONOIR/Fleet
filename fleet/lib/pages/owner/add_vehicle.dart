...
class _AddVehiclePageState extends State<AddVehiclePage> {
  ...
  void initState() {
    super.initState();
    if (widget.vehicleImagePaths != null &&
        widget.vehicleImagePaths!.isNotEmpty) {
  ...
  Future<String> _uploadImageToImgBB(File imageFile) async {
    ...
    try {
      if (!imageFile.existsSync()) {
        throw Exception(
          "The image file was lost from cache. Please select the photos again.",
        );
      }
      ...
      final request = http.MultipartRequest('POST', uri)
        ..files.add(
          http.MultipartFile.fromBytes(
            'image',
            bytes,
            filename: imageFile.path.split('/').last.isEmpty
                ? 'image.jpg'
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
          'Server rejected the upload. Status: ${response.statusCode}',
        );
      }
    } catch (e) {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Vehicle added successfully! Pending staff approval.',
              style: TextStyle(fontFamily: 'Poppins'),
            ),
          ...
          TextButton.icon(
            onPressed: () async {
              ...
              if (newPhotos != null && newPhotos is List<String>) {
                final permanentPhotos = await _saveToPermanentDirectory(
                  newPhotos,
                );
            ...
            GestureDetector(
              onTap: () async {
                final newPhotos = await Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (context) =>
                        const TakePhotoPage(vehicleName: 'Add Photos'),
                  ),
                );
                if (newPhotos != null && newPhotos is List<String>) {
                  final permanentPhotos = await _saveToPermanentDirectory(
