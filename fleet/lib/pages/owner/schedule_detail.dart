...
class _ScheduleDetailPageState extends State<ScheduleDetailPage> {
  ...
  Future<void> _fetchRealData() async {
    try {
      ...
      String vId =
          ...
          (widget.booking['vehicleData'] != null
              ? widget.booking['vehicleData']['id']
              : '');
  ...
  Color _getStatusColor(String status) {
    switch (status.toLowerCase()) {
      ...
      case 'using':
        return const Color.fromRGBO(172, 114, 161, 1.0);
      default:
        return Colors.black;
    }
  }
        ...
        children: [
                      ...
                      GestureDetector(
                        ...
                        child: Row(
                          children: [
                            ...
                            Text(
                              (_vehicleData['rating'] ?? 0.0).toStringAsFixed(
                                1,
                              ),
                              style: const TextStyle(
                                fontFamily: 'Poppins',
          ...
          Row(
            ...
            children: [
              _buildInfoColumn(
                'Deposit (฿)',
                (_bookingData['deposit_paid'] ?? 0).toString(),
              ),
              _buildInfoColumn(
                'Price/Hour (฿)',
                (_vehicleData['price_per_day'] ?? 0).toString(),
              ),
            ],
          ),
  ...
  Widget _buildRentImages(List<dynamic> images, String emptyText) {
    ...
    return Container(
      ...
      child: ListView.builder(
        ...
        itemCount: images.length,
        itemBuilder: (context, index) {
          String imgPath = images[index].toString().trim();
  ...
  Widget _buildBeforeRentImages() {
    List<dynamic> images = [];
    if (_bookingData['before_images'] != null &&
        (_bookingData['before_images'] as List).isNotEmpty) {
      images = _bookingData['before_images'];
    } else if (_bookingData['beforeImages'] != null &&
        (_bookingData['beforeImages'] as List).isNotEmpty) {
      images = _bookingData['beforeImages'];
    } else if (_bookingData['photos_before'] != null &&
        (_bookingData['photos_before'] as List).isNotEmpty) {
      images = _bookingData['photos_before'];
    } else if (widget.booking['beforeRentImages'] != null &&
        (widget.booking['beforeRentImages'] as List).isNotEmpty) {
  ...
  Widget _buildAfterRentImages() {
    List<dynamic> images = [];
    if (_bookingData['after_images'] != null &&
        (_bookingData['after_images'] as List).isNotEmpty) {
      images = _bookingData['after_images'];
    } else if (_bookingData['afterImages'] != null &&
        (_bookingData['afterImages'] as List).isNotEmpty) {
      images = _bookingData['afterImages'];
    } else if (_bookingData['return_images'] != null &&
        (_bookingData['return_images'] as List).isNotEmpty) {
      images = _bookingData['return_images'];
    } else if (_bookingData['handin_images'] != null &&
        (_bookingData['handin_images'] as List).isNotEmpty) {
      images = _bookingData['handin_images'];
    } else if (_bookingData['photos_after'] != null &&
        (_bookingData['photos_after'] as List).isNotEmpty) {
      images = _bookingData['photos_after'];
    } else if (widget.booking['afterRentImages'] != null &&
        (widget.booking['afterRentImages'] as List).isNotEmpty) {
  ...
  Widget _buildDynamicBottomSection(BuildContext context, String status) {
    ...
    String pendingType =
        _bookingData['pending_type'] ?? widget.booking['pendingType'] ?? 'rent';
          ...
          return Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text(
                  'The renter has returned the vehicle. Please inspect it.',
                  style: TextStyle(
                    fontFamily: 'Poppins',
                    fontSize: 14,
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: Text(
                  _bookingData['reject_reason'] ??
                      _bookingData['cancel_reason'] ??
                      '-',
                  style: const TextStyle(
                    fontFamily: 'Poppins',
                    fontSize: 13,
  ...
  Widget build(BuildContext context) {
    ...
    String status =
        _bookingData['status'] ?? widget.booking['status'] ?? 'Pending';
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              ...
              _showConfirmRejectModal(
                pageContext,
                reasonController.text,
                widget.booking['renterName'] ?? 'the renter',
              );
            },
            child: const Text(
              'Next',
  ...
  void _showConfirmRejectModal(
    ...
    showDialog(
      ...
      builder: (confirmDialogContext) => AlertDialog(
        ...
        content: Text(
          'Are you sure you want to decline $renterName\'s request?\n\nYour Reason:\n"$reason"',
          style: const TextStyle(fontFamily: 'Poppins', fontSize: 13),
        ),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              try {
                ...
                double refundAmount =
                    (_bookingData['deposit_paid'] ?? 0).toDouble() +
                    (_bookingData['total_price'] ?? 0).toDouble();

                await FirebaseFirestore.instance
                    .collection('bookings')
                    .doc(bId)
                    .update({'status': 'cancel', 'cancel_reason': reason});
                ...
                await FirebaseFirestore.instance
                    ...
                    .add({
                      ...
                      'status': 'success',
                      'description':
                          'Refund for cancelled booking (Owner declined)',
                    });
                ...
                await FirebaseFirestore.instance.collection('notifications').add({
                  ...
                  'message':
                      'Your rental request was declined. Reason: $reason. A full refund of ฿$refundAmount has been issued to your wallet.',
                  'is_read': false,
                ...
                if (pageContext.mounted) {
                  ...
                  ScaffoldMessenger.of(pageContext).showSnackBar(
                    const SnackBar(
                      content: Text(
                        'Rejection sent and refund processed.',
                ...
                if (confirmDialogContext.mounted)
                  Navigator.pop(confirmDialogContext);
                if (pageContext.mounted) {
  ...
  void _showAcceptModal(BuildContext pageContext) {
    showDialog(
      ...
      builder: (acceptDialogContext) => AlertDialog(
        ...
        content: Text(
          'Do you confirm to rent to ${widget.booking['renterName'] ?? 'the renter'}?',
          style: const TextStyle(fontFamily: 'Poppins', fontSize: 13),
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(acceptDialogContext),
            child: const Text(
              'Cancel',
              style: TextStyle(
                fontFamily: 'Poppins',
            ...
            onPressed: () async {
              ...
              try {
                ...
                await FirebaseFirestore.instance.collection('notifications').add({
                  ...
                  'message':
                      'Your rental request for ${_vehicleData['vehicle_name'] ?? 'the vehicle'} has been accepted by the owner.',
                  'is_read': false,
                  'created_at': FieldValue.serverTimestamp(),
                });
                ...
                if (acceptDialogContext.mounted)
                  Navigator.pop(acceptDialogContext);
                if (pageContext.mounted) {
                  ScaffoldMessenger.of(
                    pageContext,
